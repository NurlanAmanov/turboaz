...
for (let i = 0; i < data.length; i++) {
  container.innerHTML += `  <div class="card overflow-hidden w-2/5 max-w-52 text-start  h-40 min-h-80 rounded-xl bg-white  pb-3.5">
...
const bannov = document.getElementById('bannov')
const exchange = document.getElementById('exchange')
const markaselecetmbil = document.getElementById('markaselecetmbil')
const yeni=document.getElementById('yeni')
const surulumus=document.getElementById('surulumus')
const markalar = new Set();
const modeler = new Set();
const citys = new Set();
const bantyp = new Set()
const valyuta = new Set()
const mobilmark=new Set()
for (let j = 0; j < data.length; j++) {
  markalar.add(data[j].brand);
  modeler.add(data[j].model);
  citys.add(data[j].city)
  bantyp.add(data[j].banType)
  valyuta.add(data[j].currency)
  mobilmark.add(data[j].brand)



}
const marka = Array.from(markalar)
const model = Array.from(modeler)
const seherler = Array.from(citys)
const bantyape = Array.from(bantyp)
...
function addvalyuta() {
  exchange.innerHTML = '';
  for (let j = 0; j < deyisvalyuta.length; j++) {
    exchange.innerHTML += `<option>${deyisvalyuta[j]}</option>`
  }
...
function addban() {
  bannov.innerHTML = '';
  for (let j = 0; j < bantyape.length; j++) {
    bannov.innerHTML += `<option>${bantyape[j]}</option>`
  }
...
function addcity() {
  city.innerHTML = '';
  for (let j = 0; j < seherler.length; j++) {
    city.innerHTML += `<option>${seherler[j]}</option>`
  }
...
function yenideyis(){
yeni.style.backgroundColor="#ca1016"
yeni.style.color="#fff"
}
function sur(){
  surulumus.style.backgroundColor="#ca1016"
  surulumus.style.color="#fff"
}
// function showcar(e) {
//   e.preventDefault();
//   const container = document.getElementById('container');
